...
#[test]
fn iwyu_library_apply_exercises_all_policies() {
    let src = fixture_path("iwyu-library");
    let dst = tmp();
    copy_dir(&src, &dst);
    ...
    assert!(
        after.contains("\"mylib/internal/foo.h\"  /* IWYU pragma: keep */"),
        "got:\n{after}"
    );
    // fill_if_absent with existing user comment: keep user note verbatim.
    assert!(after.contains("\"mylib/private/bar.h\"  // user note"));
    assert!(
        !after.contains("bar.h\"  // user note  //")
            && !after.contains("bar.h\"  // IWYU pragma: private"),
    ...
    assert!(after.contains("\"mylib/legacy/old.h\"  // IWYU pragma: export"));
    assert!(
...
#[test]
fn iwyu_library_apply_is_idempotent() {
    ...
    let after = fs::read_to_string(dst.join("src/main.c")).unwrap();
    // No duplicated IWYU pragmas anywhere.
    assert_eq!(
        after.matches("IWYU pragma: keep").count(),
        1,
        "prepend should not duplicate; got:\n{after}"
    );
    assert_eq!(
        after.matches("IWYU pragma: associated").count(),
        1,
        "append should not duplicate; got:\n{after}"
    );
    assert_eq!(
        after.matches("IWYU pragma: private").count(),
        1,
        "fill_if_absent should not duplicate; got:\n{after}"
    );
    assert_eq!(
        after.matches("IWYU pragma: export").count(),
        1,
